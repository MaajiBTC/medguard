...
- Frontend: React + three.js
  - three.js is scoped to two places only: the **Security Dashboard visualization** and the **login/landing page**. Do not use it in the functional record-access screens (search, category views, etc.) — keep those plain React for clarity and performance. Login page's three.js piece implemented 2026-08-29 (`LoginScene.jsx`, revised twice same day): a plum shield body (extruded `THREE.Shape`, its own silhouette — not a coin) with a hospital cross emblem extruded on its front face — the entrance spins it a full 360° (decelerating ease) while it pops in (overshoot ease on scale), revealing the shield's plain back before it settles front-facing (cross forward); idles afterward with a gentle sway + bob and an orbiting lavender ring. Respects `prefers-reduced-motion`. Was a bare placeholder div before 2026-08-29.
