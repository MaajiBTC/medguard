...
- Frontend: React + three.js
  - three.js is scoped to two places only: the **Security Dashboard visualization** and the **login/landing page**. Do not use it in the functional record-access screens (search, category views, etc.) — keep those plain React for clarity and performance. Login page's three.js piece implemented 2026-08-29 (`LoginScene.jsx`, revised same day): a plum coin-shaped medallion (`THREE.CylinderGeometry`) with a hospital cross extruded on its front face and a shield extruded on its back — the entrance spins it a full 360° (decelerating ease) while it pops in (overshoot ease on scale), so both emblems are revealed before it settles front-facing (cross forward); idles afterward with a gentle sway + bob and an orbiting lavender ring. Respects `prefers-reduced-motion`. Was a bare placeholder div before 2026-08-29.
